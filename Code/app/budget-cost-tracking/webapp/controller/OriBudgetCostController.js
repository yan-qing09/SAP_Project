sap.ui.define([
    ...
    return Controller.extend("budgetcosttracking.controller.BudgetCostPage", {
        ...
        onContinuePress: function () {
            ...
            // Check if any WBS element has a variance overrun (variance > 0)
            var overrunExists = aWBS.some(function (item) {
                return parseFloat(item.Variance) > 0; // Variance overrun condition (ensure to parse to float)
            });
            ...
            if (overrunExists) {
                ...
                sap.m.MessageBox.confirm(
                    "There are budget overruns. Would you like to readjust the resource allocation?", {
                        ...
                        onClose: this._onMessageBoxClose.bind(this) // Handle the response
                    }
                );
            } else {
            ...
            this._saveActualCostToDatabase(aWBS, aWBS.ActualCost);
        },
        ...
        _saveActualCostToDatabase: function(aWBS, newActualCost) {
            var oModel = this.getView().getModel(); // Assuming the OData v4 model is set at the view level
        
            // Use the aWBS parameter to get the binding context
            var oBindingContext = oModel.getContext("/WBSSet(" + aWBS.ProjectID + ",'" + aWBS.WBS + "')");
        
            if (oBindingContext) {
                // Modify the properties of the entity
                oModel.setProperty("ActualCost", newActualCost, oBindingContext);
        
                // After modifying, submit changes to the backend
                oModel.submitChanges({
                    success: function (oData, response) {
                        sap.m.MessageToast.show("Actual cost updated successfully.");
                    },
                    error: function (oError) {
                        sap.m.MessageToast.show("Error updating actual cost.");
